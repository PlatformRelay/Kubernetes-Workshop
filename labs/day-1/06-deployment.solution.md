# Lab 06 — Deployment (S06) — solutions
...
If the rollout remains stalled, run
`kubectl rollout undo deployment/web -n "$NS"` and wait for rollout status. Describe the
newest ReplicaSet and its Pods before retrying.
