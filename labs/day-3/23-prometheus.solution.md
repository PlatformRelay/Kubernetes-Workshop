# Lab 23 — Prometheus Operator (S23) — solutions
...
The Prometheus Operator turns ServiceMonitor objects into scrape config. Two
independent selectors must both match: which Services the monitor selects, and which named
port to scrape. Either layer failing yields an empty target list even when Pods are Ready.
...
# fix selector/labels to match the Service, then:
...
Before the fix, the scrape pool is missing or 0/0. After selector/port alignment,
Endpoints are populated and Prometheus lists an up target for the app.
...
Compare serviceMonitor.spec.selector with Service labels and endpoints.port with the
