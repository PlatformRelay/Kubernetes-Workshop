...
| --- | --- | --- | --- |
| **S08** Ingress ([lab](../labs/day-1/08-ingress.md)) | **Ingress controller** (Contour) — profile `ingress-contour` | Nothing serves an Ingress until a controller exists; the lab exposes the red-line app north-south. **Mutually exclusive** with `gateway-envoy` (never install both). | Prefer `./workshop profile ingress-contour` (or `make profile-ingress-contour`). Manual: Contour v1.33.5 pinned quickstart. kind needs the ingress-ready 80/443 port mappings — the repo's kind cluster config already has them. |
| **S09** Gateway API ([lab](../labs/day-2/09-gateway-api.md)) | **Gateway API standard CRDs + Envoy Gateway** — profile `gateway-envoy` (canonical) | The Gateway API is CRD-based; you need the standard-channel CRDs **and** a controller that owns a `GatewayClass`. **Mutually exclusive** with Contour. | Prefer `./workshop profile gateway-envoy` (or `make profile-gateway-envoy`). Manual: Gateway API `standard-install.yaml` (v1.5.1) then Envoy Gateway `install.yaml` (v1.8.2). Provides the `eg` GatewayClass — labs must set `gatewayClassName: eg` explicitly. |
