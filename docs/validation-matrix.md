...
| [`day-3/21-gitops-flux.md`](../labs/day-3/21-gitops-flux.md) | GitOps — Flux variant | kind ✓ (installs Flux) / shared NS: read-only | **Flux** | Flux [`install.yaml` — **unpinned (`latest` release)**](https://github.com/fluxcd/flux2/releases/latest/download/install.yaml) (live smoke saw **v2.9.3**); app repo [`argoproj/argocd-example-apps` guestbook](https://github.com/argoproj/argocd-example-apps.git) — same hostless source as Argo variant; evidence [`us-gitops-choice-c-flux-live-smoke.md`](./validation-evidence/us-gitops-choice-c-flux-live-smoke.md) | `kind-smoke` |
